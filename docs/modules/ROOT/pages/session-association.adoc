...
**Tool Runs** (xref:tool-run-diagnostics.adoc[tool-run diagnostics]) register as additional surfaces on the same association bus: a compile run collapses to a tile, keeps `shellSessionId`, and asks the host to `focusSurface` / flash when the user follows a magic link from the CLI stub.
