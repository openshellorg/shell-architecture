...
Hive audit (2026-08-28): dlang-supplemental already has a **2D GPU** path (ello-d / Vello+wgpu) and a **retained-mode GUI** (dew + app-kit dui), plus alternate desktop bindings (valonia-d) and in-terminal TUI bindings (opentui-d). Still **missing** for a greenfield terminal app: ConPTY/PTY host, VT/emulation surface, and **whole-UI DPI/scale** helpers.

Windows Terminal Ctrl+= zooms shell contents only — inferior if the goal is consistent scale of chrome + PTY together. That tension is **recorded**, not resolved: committed shell-host remains the WT fork; dew/vello are enabling tech if a later decision needs D-hosted chrome or whole-UI scale. Do not start a D terminal emulator until PTY + scale are scoped.
