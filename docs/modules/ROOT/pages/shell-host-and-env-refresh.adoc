...
Hive audit (2026-08-28, DPI note updated same day): dlang-supplemental already has a **2D GPU** path (vello-d / Vello+wgpu) and a **retained-mode GUI** (dew + app-kit dui), plus alternate desktop bindings (avalonia-d) and in-terminal TUI bindings (opentui-d). **Whole-UI DPI/content scale** now lives in link:https://github.com/dlang-supplemental/dew[`dew.scale`] / `App.contentScale` (logical layout → scaled display list; GLFW + Win32 DPI probes). ConPTY/PTY **host bindings** are addressed by link:https://code.dlang.org/packages/pty-d[dlang-supplemental/pty-d] on DUB (Windows ConPTY + POSIX `posix_openpt`; thin FFI only). VT/ANSI → cell-grid **screen model** is link:https://code.dlang.org/packages/vt-d[dlang-supplemental/vt-d] on DUB (pure logic; no PTY/window/GPU). Still **missing** for a production greenfield terminal: live ConPTY IO + multi-window index chrome + env-refresh parity.
Scaffold started: link:https://github.com/openshellorg/open-terminal[`openshellorg/open-terminal`] (decoupled session-index product model; headless `vt-d` spike; dew pane spike). See xref:open-terminal-host.adoc[open-terminal host].
Prior "window piping" dlangui code: **NOT FOUND** -- product seed is the decoupled calling-window index (tabs / thumbnail grid) vs separate session OS windows.

Windows Terminal Ctrl+= zooms shell contents only — inferior if the goal is consistent scale of chrome + PTY together. That tension is **recorded**, not resolved: committed shell-host remains the WT fork; dew/vello are enabling tech if a later decision needs D-hosted chrome or whole-UI scale. PTY + VT libraries exist (`pty-d`, `vt-d`); DPI/scale foundation is in dew. **Committed** near-term host remains the WT fork. Parallel greenfield scaffold: xref:open-terminal-host.adoc[open-terminal] -- keep the fork until env-refresh + host-chrome parity.
