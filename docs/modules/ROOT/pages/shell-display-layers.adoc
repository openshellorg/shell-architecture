...
== Syntax mode swap (UNIX vs Nushell)

Open Shell keeps **one session** and swaps the **parser/evaluator mode**:
...
| *UNIX* (POSIX-ish)
| Pipes, `$VAR`, `&&`, heredocs, classical syntax — eval path similar to bash/fish shim

| *Structured* (Nushell + OSO)
| Nu tables, `$env`, OSO context tokens, structured pipelines
|===

Swap without spawning a nested shell in another PTY:

* **UI:** always-on toggle in host chrome — single control (dropdown) in the `Ctrl+1` slot, not two adjacent mystery buttons
* **Keyboard:** `Ctrl+PgUp` / `Ctrl+PgDn` cycle modes (PgUp/PgDn alone reserved for overlay scroll)
* Mode indicator in prompt context zone (grey token: `mode: nu` / `mode: unix`)

Rationale for **collapsed toggle** vs `Ctrl+1`/`Ctrl+2` side by side: two adjacent mode keys imply parity weight; a selector communicates one session, one parser active.
...
Prohelp integrates at the **base + context** layers:

* `help` / `?` routes through prohelp for PATH topics
* Context token focused → prohelp scopes help to that namespace
...
* `openshellorg/nushell` — layers, control plane, paste preview, mode swap
