...
The always-on **top context bar** prefers **native host chrome** when the parent terminal accepts it — **inserted** into host chrome, not hovered over the shell.
Otherwise it renders on this page's **display overlay** stack.
See xref:host-chrome-negotiation.adoc[host chrome negotiation] (probe / offer / accept/reject / render).

App top bars and hover widgets (color picker, GUI editor, file upload, ...) are a separate channel: xref:host-native-widgets.adoc[host native widgets].
...
That upgrades shell *data*. Compilers and long adversarial diagnostics belong on xref:tool-run-diagnostics.adoc[Tool Runs] instead. Per-command xref:command-channels.adoc[channel switchers] keep stdout / stderr un-weaved while either path runs.
...
* link:https://hci-nerdz.github.io/docs/hci-nerdz/programmer-pov-diagnostics.html[Programmer-POV diagnostics @ HCI Nerdz]
